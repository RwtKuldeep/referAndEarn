...
<body>
    <?php include 'pack/SideNav.php'; ?>
        ...
        <div class="container-fluid family">
            ...
            <div class="row">
                <div class="col-md-12 ">
                    <div class="row  mt-5">
                      <div class="col-8"><h3 class="family"><b>All Withdrawal Request</b></h3></div>
    ...
    <?php include("pack/footer.php"); ?>
     <form id="statusForm" method="get" action="php/withdraw/statusChange.php">
  ...
  function PaymentAmountFun(id)
  {
    ...
    <script>
      ...
      'click .like': function(e, value, row, index) {
        let id = JSON.stringify(parseInt(row.uid));
        location.href = "workreport.php?id=" + id;
            ...
            'click .remove': function (e, value, row, index) 
            {
                ...
                delete_confirm("php/withdraw/delete.php?id="+id);
            }
}
    ...
    //   ModelOpenCreditAdd End ------------------------------------------------
      ...
      function initTable() 
      {
                    ...
                    },
                    {
                        ...
                        // sortable: true
                         ...
                         formatter: function (value, row, index) {
                             // console.log(row.status_change);
                           return [
                             '<select class="form-control status-select" onchange="changeStatus(this.value, ' + row.id + ')">',
                             '<option value="0"' + (row.status_change == 0 ? 'selected' : '') + '>pending</option>',
                             '<option value="1"' + (row.status_change == 1 ? 'selected' : '') + '>Accept</option>',
                             '<option value="2"' + (row.status_change == 2 ? 'selected' : '') + '>Declined</option>',
                             '</select>'
        ...
        $remove.click(function () {
          ...
          delete_confirm("php/withdraw/delete.php?id="+ids);
        })
      }
