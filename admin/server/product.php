<?php
    ...
    {
        $search = $_GET['search'];
        $query_search = "where id LIKE '%$search%' OR trending LIKE '%$search%'";
    }

    $result = array();
    $update = db("select * from product");
    $numOfrow = mysqli_num_rows($update);
    $result = array("total" => $numOfrow,"totalNotFiltered" => $numOfrow,);
    $rrr = array();
    $update1 = db("select * from product {$query_search} {$query_sort} {$query_limit}");
    if ($numOfrow > 0) 
    {
        $i = 1;
